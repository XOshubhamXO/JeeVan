...
export default function HomePage() {
  ...
  const VENTURES = [
    { id:'nursery', name:t('venture.nursery'), desc:t('venture.nursery.desc'), icon:<Sprout className="w-5 h-5"/>, img:'/nalanda-nursery.jpg', color:'var(--accent-green)' },
  ...
  const STATS = [{ value:'164+', label:t('stats.plants') },{ value:'13', label:t('stats.partners') },{ value:'30', label:t('stats.countries') },{ value:'$0', label:t('stats.cost') }]
        ...
        <AnimatePresence>
          {heroVisible && (
            <motion.div key="hero" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0,y:-30}} transition={{duration:0.6}}>
              <ParallaxHero image="/nalanda-aerial.jpg" video={process.env.NEXT_PUBLIC_CLOUDINARY_VIDEO_URL} speed={0.35}>
                <a href="#main-content" className="skip-link">{t('hero.skip')}</a>
                <header className="px-6 md:px-10 py-5 flex items-center justify-between" role="banner">
                  ...
                  <button onClick={startOnboarding} className="btn-ghost text-sm text-white/60 hover:text-white" aria-label={t('nav.skip')}>{t('nav.skip')}</button>
                </header>
                <div className="flex-1 flex flex-col items-center justify-center px-6 text-center h-full">
                  <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{delay:0.3,duration:0.8}}>
                    <h1 className="text-white mb-4 whitespace-pre-line" style={{fontFamily:'var(--font-display)',fontSize:'clamp(3rem, 6vw, 5rem)',lineHeight:1.1}}>{t('hero.headline')}</h1>
                    <p className="lead max-w-lg mx-auto text-white/70 mb-4">{t('hero.subtitle')}</p>
                    ...
                    <button onClick={scrollToCountry} className="btn-primary text-base px-8 py-4 rounded-full">{t('hero.cta')} <ArrowDown className="w-4 h-4 ml-1"/></button>
                  </motion.div>
                </div>
          ...
          <div className="relative z-10" ref={countryRef} role="main" id="main-content">
            <NatureEngine bgImage="/bg-gradient-4k.jpg">
                ...
                <header className="px-6 md:px-10 py-5 flex items-center justify-between">
                  ...
                  {(['country','language','survey'] as Step[]).map((s,i) => s!=='hero' && (
                    <div key={s} className="flex items-center gap-2">
                      ...
                      <span className={`text-[11px] uppercase tracking-wider ${i-1===['country','language','survey'].indexOf(step)?'text-green-400':i-1<['country','language','survey'].indexOf(step)?'text-white/40':'text-white/20'}`}>
                        {[t('onboarding.country'),t('onboarding.language'),t('onboarding.profile')][i-1]}
                      </span>
                    </div>
                  ))}
                </header>
              ...
              <section className="py-24 md:py-32 px-6 md:px-10" style={{background:'var(--bg-secondary)'}}>
                <div className="max-w-7xl mx-auto">
                  <div className="text-center mb-16">
                    ...
                    <h2 id="ventures-heading" style={{fontFamily:'var(--font-display)'}} className="mt-3">{t('ventures.heading')}</h2>
                    <p className="lead max-w-xl mx-auto mt-4">{t('ventures.subtitle')}</p>
                  </div>
                    ...
                    {VENTURES.map((v,i)=>(
                      <motion.div key={v.id} initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:"-80px"}} transition={{delay:i*0.12,duration:0.6}}
                        ...
                        <div className="p-6">
                          ...
                          <span className="inline-flex items-center gap-1 mt-4 text-sm font-medium" style={{color:'var(--accent-green)'}}>{t('ventures.learn_more')} <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform"/></span>
                        </div>
                      </motion.div>
                    ))}
                  </div>
              ...
              <section className="py-20 px-6 md:px-10">
                <div className="max-w-7xl mx-auto">
                  <div className="text-center mb-12"><span className="label" style={{color:'var(--accent-gold)'}}>{t('partners.label')}</span><h2 style={{fontFamily:'var(--font-display)'}} className="mt-3">{t('partners.heading')}</h2></div>
                  ...
                  <div className="text-center"><span className="label" style={{color:'var(--accent-green)'}}>{t('causes.label')}</span><h2 style={{fontFamily:'var(--font-display)'}} className="mt-3 mb-8">{t('causes.heading')}</h2>
